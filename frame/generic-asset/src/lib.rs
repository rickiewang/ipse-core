...
//! ```
//! use support::{
//! 	traits::{Currency, ExistenceRequirement, WithdrawReason, Get,},
//! 	dispatch::Result,
//! };
...
use sp_runtime::traits::{
	...
	Zero, Bounded, EnsureOrigin,
};

use rstd::prelude::*;
use rstd::{cmp, result, fmt::Debug};
use support::dispatch::Result;
use support::{debug,weights::{SimpleDispatchInfo},
	decl_event, decl_module, decl_storage, ensure, StorageValue, StorageMap, StorageDoubleMap, Blake2_256,
	traits::{
		Currency, ExistenceRequirement, Imbalance, LockIdentifier, LockableCurrency, ReservableCurrency,
		SignedImbalance, UpdateBalanceOutcome, WithdrawReason, WithdrawReasons, TryDrop, Get,
	},
	Parameter,
};
...
pub trait Trait: system::Trait {
	// 铸币申请需要抵押的金额
	type MintSpend: Get<u128>;
	// 销毁币需要抵押的金额
	type BurnSpend: Get<u128>;
	// 一次铸币的最小金额
	type MintMinAmount: Get<u128>;
	// 一次销毁币的最小金额
	type BurnMinAmount: Get<u128>;
	// 铸币议案存在的时间长短
	type MintLong: Get<Self::BlockNumber>;
	// 铸币周期长短（多久统一铸币一次）
	type MintPeriod: Get<Self::BlockNumber>;
	// 销毁币的周期（多久集体销毁一次）
	type BurnLong: Get<Self::BlockNumber>;
...
impl<T: Trait> Subtrait for T {
	type MintSpend = T::MintSpend;
	type BurnSpend = T::BurnSpend;
	type MintMinAmount = T::MintMinAmount;
	type BurnMinAmount = T::BurnMinAmount;
	type MintLong = T::MintLong;
	type MintPeriod = T::MintPeriod;
	type BurnLong = T::BurnLong;
	type Currency3 = T::Currency3;
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn create(origin, options: AssetOptions<T::Balance, T::AccountId>) -> Result {
			...
			// 应该是有更新权限的人  可以给不同的人分派不同的权限 但是一个权限只能一个人
		...
		// 矿工申请铸币
		#[weight = SimpleDispatchInfo::FixedNormal(500_000)]
		fn need_mint(origin, asset_id: T::AssetId, amount: T::Balance) -> Result{
			let who = ensure_signed(origin)?;

			// 铸币金额太小 不给铸币
			if let Some(t) = amount.try_into().ok() {
				let amount_1 = t.try_into().unwrap();
				ensure!(T::MintMinAmount::get() <= amount_1, "you amount too low");
			}
			else{
				return Err("amount is none");
			};

			// 如果正在等待的铸币队列大于100， 则不给申请铸币
			let tempt_queue = Self::vote_queue();
			ensure!(tempt_queue.len() as u32 <= 100, "the mint queue too long now, please waitting for a second ");

			// 同一种币同一时间只能提一个议案
			ensure!(!(<MintVoteInfo<T>>::exists((who.clone(), asset_id.clone()))), "you can't mint this asset twice in the same time ");

			// 抵押金额（不够不给操作）
			let bond = <BalanceOf<T> as TryFrom<u128>>::try_from(T::MintSpend::get());
			if let Some(t) = bond.ok(){
				T::Currency3::reserve(&who, t)
				.map_err(|_| "Proposer's balance too low")?;
			} else{
				return Err("you mint spend is None");
			};
		...
		#[weight = SimpleDispatchInfo::FixedNormal(500_000)]
		fn council_vote_for_mint(origin, who: T::AccountId, asset_id: T::AssetId, appro_or_reject: Vote) -> Result{
			// 议会成员才有投票的资格
			let origin = T::CouncilOrigin::ensure_origin(origin)?;

			let mut vote = Self::mint_vote_info((who.clone(), asset_id.clone())).ok_or("vote must exists")?;
			...
			if appro_or_reject == Vote::Approve{
			...
			// todo 这里要计算议会成员的总人数 然后乘于百分比
			if reject_votes > 3u32{
				Self::mint_last_do(vote)?;
			}
			else{
				<MintVoteInfo<T>>::insert((who.clone(), asset_id.clone()), vote);
			}
		...
		#[weight = SimpleDispatchInfo::FixedNormal(500_000)]
		fn Technical_reject_mint(origin, who: T::AccountId, asset_id: T::AssetId) -> Result{
			// 技术委员会成员才有一票否决的资格
			let origin = T::TechnicalOrigin::ensure_origin(origin)?;

			// 这个议案要存在
			let mut vote = Self::mint_vote_info((who.clone(), asset_id.clone())).ok_or("vote must exists")?;
			vote.technical_reject = true;

			Self::mint_last_do(vote.clone())?;
			Ok(())
		...
		#[weight = SimpleDispatchInfo::FixedNormal(500_000)]
		fn burn(origin, #[compact] asset_id: T::AssetId, amount: T::Balance) -> Result {
			...
			ensure!(!(<BurnStatics<T>>::exists((to.clone(), asset_id.clone()))), "you can't burn this asset twice in the same time");

			// 同一时间 不能超过100个销毁申请
			let queue = <BurnQueue<T>>::get();
			let queue_len = queue.len() as u32;
			ensure!(queue_len <= 100, "wait a second, the burn queue too lonng");

			// 销毁币金额太小 不给销毁币
			if let Some(t) = amount.try_into().ok() {
				let amount_1 = t.try_into().unwrap();
				ensure!(T::BurnMinAmount::get() <= amount_1, "you amount too low");
			}
			else{
				return Err("amount is none");
			};
			...
			// 抵押金额不够不给操作
			let bond = <BalanceOf<T> as TryFrom<u128>>::try_from(T::BurnSpend::get());
			if let Some(t) = bond.ok(){
				T::Currency3::reserve(&to, t)
				.map_err(|_| "Proposer's balance too low")?;
			} else{
				return Err("you burn spend is None");}
		...
		#[weight = SimpleDispatchInfo::FixedNormal(500_000)]
		fn foundation_tag_for_burn(origin, who: T::AccountId, asset_id: T::AssetId) -> Result{
			// 只有基金会成员才有权限
			let origin = T::TransxFoundation::ensure_origin(origin)?;
			let mut burn_info = Self::burn_statisc((who.clone(), asset_id.clone())).ok_or("burn info must exists")?;
			burn_info.foundation_tag_man = Some(origin.clone());

			// 惩罚掉抵押
			let bond = <BalanceOf<T> as TryFrom<u128>>::try_from(T::BurnSpend::get());
			let mut bond_1 = <BalanceOf<T>>::from(0u32);
			let real_bond = if let Some(t) = bond.ok(){
				 t
					}
			else{
				return Err("you burn spend is None");};
			...
			// slash掉申请铸币者的抵押币
			T::Currency3::slash_reserved(&who, real_bond.clone());

			// 奖励抵押金额给基金会贴上标签的人
			T::Currency3::deposit_creating(&origin, real_bond);

			Self::burn_last_do(burn_info)?;
			Ok(())
		}
...
decl_storage! {
	trait Store for Module<T: Trait> as GenericAsset {
		...
		pub MintVoteInfo get(fn mint_vote_info): map (T::AccountId, T::AssetId) => Option<MintVote<T::AccountId,
		T::AssetId, T::Balance, T::BlockNumber>>;

		// 一个二级map，用于存储所有铸币议案的最终信息（永久存储）
		pub MintVotoInfo2 get(fn mint_vote_info_2): double_map hasher(twox_64_concat) T::AccountId, blake2_256(T::AssetId) => Vec<MintVote<
		...
		pub BurnStatics get(fn burn_statisc): map (T::AccountId, T::AssetId) => Option<BurnInfo<T::AccountId,
		T::AssetId, T::Balance, T::BlockNumber>>;

		// 永久存储销毁议案的信息
		pub BurnStatics2 get(fn burn_statisc_2): double_map hasher(twox_64_concat) T::AccountId, blake2_256(T::AssetId) => Vec<BurnInfo<
		T::AccountId,T::AssetId, T::Balance, T::BlockNumber>>;
...
impl<T: Trait> Module<T> {
	...
	fn find_expire_mint_vote(n: T::BlockNumber){
		...
		if vote_queue_1.len() != 0{
			...
			|vote1| {
				...
				if n - vote.start_block.clone() >= T::MintLong::get(){
					if vote.approve_list.len() < 10{
						if Self::mint_last_do(vote.clone()).is_ok(){
							false
						}
						else{
							// 一般也不会执行
	...
	fn find_pass_vote_and_mint(n: T::BlockNumber){
		...
		if vote_queue.len() != 0{
			...
			|vote1| {
				...
				if n - vote.start_block.clone() >= T::MintLong::get(){
					// 这里不能终结
					if Self::mint(vote.asset_id.clone(), vote.mint_man.clone(), vote.amount).is_ok() && Self::mint_last_do(vote.clone()).is_ok(){
						false
					}
					else{
						true
	...
	fn find_expire_burn_info(n: T::BlockNumber){
		...
		// BurnStatics
		if burn_queue.len() != 0{
		<VoteQueue<T>>::mutate(|burns| burns.retain(
		|burn1|{
			if let Some(burn) = <BurnStatics<T>>::get(burn1){
				if n - burn.start_block.clone() >= T::BurnLong::get(){
					// 过期删除掉 银行没贴标签，失败需要重新给该用户铸币（因为用户已经先销毁）
					if Self::burn_last_do(burn.clone()).is_ok() && Self::mint(burn.asset_id.clone(), burn.burn_man.clone(), burn.amount.clone()).is_ok(){
						false
					}
					else{
						true
	...
	// 铸币最终一步（不管成功或是失败均会执行
	pub fn mint_last_do(vote: MintVote<T::AccountId, T::AssetId, T::Balance, T::BlockNumber>) -> Result{
		// 把u128转换成BalanceOf
		let bond = <BalanceOf<T> as TryFrom<u128>>::try_from(T::MintSpend::get());
		let mut bond_1 = <BalanceOf<T>>::from(0u32);
		let real_bond = if let Some(t) = bond.ok(){
		...
		// slash掉申请铸币者的抵押币
		T::Currency3::slash_reserved(&vote.mint_man, real_bond.clone());
		...
		// 获取每一个议会成员的奖励
		let per_man_reward = bond_1.clone()/man_real_count.clone();
		// 奖励每一个议会成员
		for i in 1..mans_count{
			if let Some(man) = all_mans_cp.next(){
				T::Currency3::deposit_creating(&man, per_man_reward);
			}
		}
		...
		// 添加二级map的信息
		if <MintVotoInfo2<T>>::exists(who.clone(), asset_id.clone()){
			let mut vote_vec = <MintVotoInfo2<T>>::get(who.clone(), asset_id);
			vote_vec.push(vote.clone());
			<MintVotoInfo2<T>>::insert(who.clone(), asset_id.clone(), vote_vec);
		}
		else{
	...
	fn burn_last_do(burn_info: BurnInfo<T::AccountId, T::AssetId, T::Balance, T::BlockNumber>) -> Result{
		...
		if <BurnStatics2<T>>::exists(burn_info.burn_man.clone(), burn_info.asset_id.clone()){
	...
	/// Get an account's free balance of an asset kind.ckNumber
	...
	fn mint(asset_id: T::AssetId, to: T::AccountId, amount: T::Balance) -> Result {
		let original_free_balance = Self::free_balance(&asset_id, &to);
		let current_total_issuance = <TotalIssuance<T>>::get(asset_id);
	...
	pub fn create_asset(
		...
		// 这个方法应该改成root权限
		// 这个方法是可以定义asetid 也可以不定义 不定义就用系统给的
		let asset_id = if let Some(asset_id) = asset_id {
			ensure!(!<TotalIssuance<T>>::exists(&asset_id), "Asset id already taken.");
			ensure!(asset_id < Self::next_asset_id(), "Asset id not available.");  // 创建的assetid自己自定义 但是一定要比之前的大（这个是不是改成不存在比较好）
			asset_id
		} else {
		...
		Self::deposit_event(RawEvent::Created(asset_id.clone(), account_id, options));
	...
	pub fn make_transfer(
		...
		if from != to {
			<FreeBalance<T>>::mutate(asset_id, from, |balance| *balance -= amount);  // 这个方法是直接在函数里面处理了 不需要出来
