...
pub struct Vote {
	pub aye: bool,  // 这个参数是个啥东西呢？
	pub conviction: Conviction,  // 这个是1,2,4,8,16,32
}
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn propose(origin,
			...
			// &[&who][..]这个写法很有意思 是vec的另一种表示方法
			<DepositOf<T>>::insert(index, (value, &[&who][..]));  // 只需要一个value accountid是vec

			// 新的公投信息记录公投发起人
			// 公投索引 公投hash
			let new_prop = (index, proposal_hash, who);  // 这个who是自己
		...
		fn emergency_cancel(origin, ref_index: ReferendumIndex) {
			T::CancellationOrigin::ensure_origin(origin)?;  // 超过一定数量的议会成员
			...
			<Cancellations<T>>::insert(h, true);  // 把已经被取消的记录在案
...
impl<T: Trait> Module<T> {
	...
	fn launch_public(now: T::BlockNumber) -> Result {
		// Vec<(PropIndex, T::Hash, T::AccountId)>; public_props的结构
